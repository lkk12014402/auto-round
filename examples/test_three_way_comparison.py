...
Rotation + MXFP4 Quantization Accuracy on Qwen3-0.6B
...
def run_autoround(model_name, tokenizer, rotation_flags, device, nsamples=128,
    ...
    """Apply rotation + MXFP4 quantization using auto-round."""
    ...
    ar = AutoRound(
        ...
        scheme="MXFP4_RCEIL", iters=0,
        nsamples=nsamples, seqlen=seqlen, device_map=device,
    )
    ar.quantize()
...
def print_comparison_table(all_results, tasks):
    ...
    print(f"Model: Qwen/Qwen3-0.6B | Rotation: Hadamard (deterministic, no training)")
    print("=" * 100)
...
def main():
    ...
    print_comparison_table(all_results, all_tasks)
