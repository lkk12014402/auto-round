...
v2 changes from v1 (test_rotation_scheme_matrix.py):
  - Uses the unified `apply_rotation()` API instead of direct SpinQuantPreprocessor
...
# Using the unified API: apply_rotation(model, SpinQuantConfig(...))
...
def apply_quarot_rotation(
    ...
    """Apply QuaRot rotation using the unified apply_rotation() API.

    This is the v2 API path — uses ``apply_rotation(model, config)`` from
    ``auto_round.algorithms.transforms`` which dispatches through the
    BaseRotation registry.
    ...
    Args:
        ...
        rotation_size: Optional custom rotation dimension. With v2, this is
            ...
            (768, 1536, 3072, 5120, 7168, 11008, 14336).
    ...
    return apply_rotation(model, config)
...
def run_single_combination(
    ...
    try:
        ...
        if rotation_name != "none" and any(rotation_flags.values()):
            logger.info(f"  Applying rotation via unified API: {rotation_name}")
            logger.info(f"    rotation_size={args.rotation_size or 'auto'}, "
                        f"random_hadamard={args.random_hadamard}, "
                        f"online_r1={args.online_r1}")
            model = apply_quarot_rotation(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Batch Rotation × Quantization Scheme Accuracy Matrix (v2 — unified API)",
...
def main():
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f"results_v2_{model_short}{iters_tag}_{timestamp}"
    ...
    logger.info(f"  API:            unified apply_rotation() (v2)")
    ...
    print(f"  API: unified apply_rotation() (v2)")
    print(f"{'═'*70}")
