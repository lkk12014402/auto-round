...
class SpinQuantRotation(BaseRotation, RotationSerializer):
    ...
    # ------------------------------------------------------------------
    # Layer-wise rotation interface
    # ------------------------------------------------------------------

    @property
    def supports_layerwise(self) -> bool:
        """SpinQuant supports layer-wise rotation for Online R1 mode."""
        return True
    ...
    def prepare_layerwise(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: ``dataloader`` forwarded if ``trainable_rotation=True``.
    ...
    def inject_buffers_on_layer(
        ...
        # Selective rotation: skip layers not in the rotated set.
        rotated_layers = getattr(model, "_rotated_layers", None)
        if rotated_layers is not None and layer_name not in rotated_layers:
            return
        ...
        try:
            ...
            if n > 0:
                ...
                if rotated_layers is not None:
                    cfg_dict["selective_rotation"] = True
                    cfg_dict["rotated_layers"] = sorted(rotated_layers)
                    cfg_dict["num_rotated_layers"] = len(rotated_layers)
