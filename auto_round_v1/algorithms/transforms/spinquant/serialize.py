...
def inject_spinquant_buffers(
    ...
    # Selective rotation: only inject on layers that were actually rotated.
    rotated_layers = getattr(model, "_rotated_layers", None)
    ...
    if config.r1 and config.online_r1_rotation:
        ...
        for name, module in model.named_modules():
            if not _is_quantlinear(module):
                continue
            if name not in r1_targets:
                continue
            # Skip if selective mode and layer was not rotated
            if rotated_layers is not None and name not in rotated_layers:
                continue
    ...
    if rotated_layers is not None:
        logger.info(
            f"[SpinQuant Serialize] Injected rotation buffers into "
            f"{n_injected} QuantLinear modules (selective: {len(rotated_layers)} layers rotated)"
        )
    else:
        logger.info(f"[SpinQuant Serialize] Injected rotation buffers into {n_injected} QuantLinear modules")
    return n_injected
...
def save_spinquant_config(
    ...
    # Store selective rotation metadata if present
    rotated_layers = getattr(model, "_rotated_layers", None)
    if rotated_layers is not None:
        spinquant_dict["selective_rotation"] = True
        spinquant_dict["rotated_layers"] = sorted(rotated_layers)
        spinquant_dict["num_rotated_layers"] = len(rotated_layers)
