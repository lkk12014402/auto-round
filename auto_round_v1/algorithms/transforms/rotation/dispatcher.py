...
def apply_hadamard_rotation(
    ...
    Args:
        ...
        data_type: Quantization data type (e.g. ``"mx_fp"``, ``"nv_fp"``,
            ``"int"``, ``"fp"``).
        compute_device: Device for inplace-backend computation. Also used by
            the transform backend for activation profiling (selective mode).
        calibration_dataloader: Optional dataloader for activation profiling
            in selective rotation auto mode.
    ...
    if backend == "inplace":
        ...
        return apply_rotation_transform(
            model, config, data_type=data_type,
