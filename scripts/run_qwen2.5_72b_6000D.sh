...
# ─── Configuration ────────────────────────────────────────────────────────────
MODEL_NAME=${MODEL_NAME:-"/data/lkk/Qwen2.5-72B-Instruct"}
...
get_save_args() {
    ...
    case "$cfg" in
        ...
        "R1+R2+R4_size128")
            ...
            ;;
	"R1+R2_size32")
            echo "--r1 --r2 --no-r3 --no-r4 --rotation-size 32"
            ;;
	"R1+R2_size128")
            echo "--r1 --r2 --no-r3 --no-r4 --rotation-size 128"
            ;;
        *)
            echo ""
            ;;
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    if [ "$SKIP_SAVE" != "1" ] && [ -z "$ACTUAL_MODEL_DIR" ]; then
        ...
        CUDA_VISIBLE_DEVICES="2" python save_rotated_models.py \
            --model "$MODEL_NAME" \
            --device "cuda:0" \
