...
# ─── Configuration ────────────────────────────────────────────────────────────
MODEL_NAME=${MODEL_NAME:-"/data/lkk/Qwen3-32B"}
...
BASELINE_RESULT_DIR="${RESULTS_DIR}/FP16_baseline"
if [ ! -f "${BASELINE_RESULT_DIR}/results.json" ]; then
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  [$(timestamp)] FP16 Baseline — vLLM eval (TP=$TP)"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    mkdir -p "$BASELINE_RESULT_DIR"

    CUDA_VISIBLE_DEVICES="$EVAL_GPUS" lm_eval \
        --model vllm \
        --model_args "pretrained=${MODEL_NAME},tensor_parallel_size=${TP},max_model_len=${MAX_MODEL_LEN},dtype=bfloat16,add_bos_token=True,gpu_memory_utilization=${GPU_MEMORY_UTILIZATION},enable_prefix_caching=False" \
        --tasks "$TASKS" \
        --batch_size "$BATCH_SIZE" \
        --output_path "$BASELINE_RESULT_DIR" \
        2>&1 | tee "${BASELINE_RESULT_DIR}.log"
...
for CONFIG in "${CONFIGS[@]}"; do
    ...
    if [ "$SKIP_SAVE" != "1" ] && [ -z "$ACTUAL_MODEL_DIR" ]; then
        ...
        CUDA_VISIBLE_DEVICES="4" python save_rotated_models.py \
            --model "$MODEL_NAME" \
            --device "cuda:0" \
