...
input activation x
    ...
    └─ Step 3: GEMM           ← 由 AUTO_ROUND_SPINQUANT_RUNTIME_BACKEND 控制
          选择: preunpack_bf16 | preunpack_fp8 | packed_fused
          │
          output
```
...
| `AUTO_ROUND_SPINQUANT_RUNTIME_BACKEND` | GEMM 计算后端 | `preunpack_bf16`, `preunpack_fp8`, `packed_fused` | `packed_fused` |
...
| 后端 | 实现 | 说明 |
|------|------|------|
| `preunpack_bf16` | 模型加载时将 MXFP4 packed weight 解量化为 dense bf16，推理时走 `torch.nn.functional.linear` → **cuBLAS GEMM** | 小 batch 最快，内存占用较高 |
...
| `preunpack_bf16` | bf16 dense (16bit) | ~64 GB | ~16 GB | 加载时完全 dequant 为 bf16，删除 packed 原始存储 |

> **注意**：`preunpack_bf16` 和 `preunpack_fp8` 在模型加载后会 **释放** packed weight 和 scale（`del layer.weight_packed`），
...
|------|--------|---------|----------|-----------|
| preunpack_bf16 | 0.054ms | **0.030ms** | 0.093ms | 0.324ms |
...
export AUTO_ROUND_SPINQUANT_RUNTIME_BACKEND=preunpack_bf16
...
**选择建议：**
- 显存充足 → `preunpack_bf16`（GEMM 最快，代价是 4x 权重显存）
...
| `preunpack_bf16` | 更接近 Quark：rotate(x) + activation qdq + frozen dense weight + `F.linear` | 已实现 |
...
```json
"runtime_backend": "preunpack_bf16"
```
...
| 项目 | 当前 `packed_fused` | 提议 `preunpack_bf16` |
|------|---------------------|--------------------------|
...
process_weights_after_loading():
    ...
    if runtime_backend == "preunpack_bf16":
...
- **如果看最像 Quark 的体感路径**：重点看 `preunpack_bf16`
