...
def test_preunpack_bf16_backend():
    ...
    cfg = SpinQuantMXFP4Config(
        ...
        online_r1=False,
        runtime_backend=RUNTIME_BACKEND_PREUNPACK_BF16,
    )
    method = SpinQuantMXFP4LinearMethod(cfg)
    layer = torch.nn.Module()
    method.create_weights(
    ...
    method.process_weights_after_loading(layer)
    # After preunpack_bf16 processing, packed weights are replaced with 1-element
    ...
    print(f"[PASS] preunpack_bf16 backend (device={device})")
