...
class SpinQuantMXFP4Config(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> "SpinQuantMXFP4Config":
        ...
        # Store selective rotation info for load-time weight compensation
        ...
        # Determine which layer type suffixes are NOT rotated (for merged-layer compensation).
        # For selective rotation: layers not in rotated_layers need their weight compensated
        # when they share a merged linear (e.g., v_proj in merged qkv_proj).
        ...
        if instance.selective_rotation and instance.rotated_layers:
            ...
            for suffix in ("v_proj", "q_proj", "k_proj", "gate_proj", "up_proj"):
                # If NO layer with this suffix appears in rotated_layers, it's unrotated
            ...
            if instance._unrotated_suffixes:
                logger.info(
                    f"[AutoRound] Selective rotation: unrotated layer types "
                    f"{instance._unrotated_suffixes} will be compensated in merged layers"
                )
...
class SpinQuantMXFP4LinearMethod(LinearMethodBase):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        # Store partition info for selective rotation compensation at load time.
        # For QKVParallelLinear: output_partition_sizes = [q_size, k_size, v_size]
        # For MergedColumnParallelLinear: [gate_size, up_size]
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # --- Selective rotation: compensate unrotated partitions in merged layers ---
    ...
    def _compensate_unrotated_partitions(self, layer: torch.nn.Module) -> None:
        ...
        When selective rotation skips certain layers (e.g., v_proj) but they are
        merged with rotated layers in vLLM (e.g., qkv_proj merges q+k+v), the
        online rotation is applied uniformly to the full input. To compensate,
        we rotate the unrotated partition's weight at load time so that:

            x_rot @ W_v_compensated.T = (x@H) @ (W_v@H).T = x @ H @ H.T @ W_v.T = x @ W_v.T

        This is mathematically exact (H is orthogonal, H@H.T = I) and has ZERO
        runtime overhead — only a one-time cost at model loading.

        The quantization benefit is preserved: v_proj was quantized with its
        ORIGINAL (unrotated) weight distribution. The rotation is applied AFTER
        dequantization, so v_proj gets the best of both worlds.
        """
        ...
        # Only compensate if this layer actually has rotation enabled
        rot_size = getattr(layer, "_r1_rot_size", 0)
        if rot_size == 0:
            return

        # Only for merged layers (3 partitions = QKV, 2 partitions = gate_up)
        partition_sizes = getattr(layer, "_output_partition_sizes", None)
        if partition_sizes is None or len(partition_sizes) <= 1:
            return

        # Determine which partitions need compensation
        # QKV (3 partitions): [q, k, v] → check if v_proj is unrotated
        # gate_up (2 partitions): [gate, up] → both typically rotated
        unrotated_suffixes = self.quant_config._unrotated_suffixes
        if len(partition_sizes) == 3:
            # QKV merged: partition order is [q, k, v]
            partition_suffixes = ["q_proj", "k_proj", "v_proj"]
        elif len(partition_sizes) == 2:
            # gate_up merged: partition order is [gate, up]
            partition_suffixes = ["gate_proj", "up_proj"]
        else:
            return

        # Find which partition indices need compensation
        compensate_indices = []
        for i, suffix in enumerate(partition_suffixes):
            if suffix in unrotated_suffixes:
                compensate_indices.append(i)

        if not compensate_indices:
            return

        # Get the rotation matrix for compensation
        ...
        # Apply compensation to the appropriate backend's weight tensor
        backend = self.quant_config.runtime_backend
        if backend == RUNTIME_BACKEND_PREUNPACK_BF16:
            weight = layer.weight_dense_qdq.data
        elif backend == RUNTIME_BACKEND_PREUNPACK_FP8:
            # For FP8 backend, compensate on the FP8 weight (less precise but functional)
            weight = layer.weight_unpacked_fp8.data
        elif backend == RUNTIME_BACKEND_PACKED_FUSED:
            # Cannot compensate packed weights without re-packing
            logger.warning_once(
                "[AutoRound] Selective rotation with packed_fused backend: "
                "cannot compensate unrotated partitions. Use preunpack_bf16 backend."
            )
            return
        else:
            return

        # Compute row ranges for each partition
        ...
        for i, psize in enumerate(partition_sizes):
            if i in compensate_indices:
                # Apply block rotation to this partition's rows:
                # W_comp = W @ block_diag(R, R, ..., R)
                ...
                if in_features % rot_size == 0:
                    ...
                    # R is [rot_size, rot_size], apply to input dim: W @ R
        ...
        if compensated_count > 0 and not getattr(self, "_logged_compensation", False):
            logger.info(
                f"[AutoRound] Selective rotation: compensated {compensated_count} "
                f"unrotated partition(s) in merged layer (zero runtime overhead)"
            )
