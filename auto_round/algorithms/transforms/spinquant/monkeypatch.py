...
class QKRotationWrapper(nn.Module):
    """Wraps ``apply_rotary_pos_emb`` to apply Hadamard R3 rotation after RoPE.

    The wrapped function is called with the same arguments as the original
    ``apply_rotary_pos_emb(q, k, cos, sin, ...)`` and applies a normalized
    Hadamard transform to both Q and K outputs.

    Math: For attention scores, (Q@H) @ (K@H).T = Q @ H @ H.T @ K.T = Q @ K.T
    since H is orthogonal (H @ H.T = I for normalized Hadamard).
    """

    def __init__(self, original_func: Callable[..., Any]):
        super().__init__()
        self.original_func = original_func
        self._had_matrix: torch.Tensor | None = None
        self._head_dim: int = 0

    def set_hadamard(self, had_matrix: torch.Tensor, head_dim: int) -> None:
        """Set the Hadamard matrix to use for rotation."""
        self._had_matrix = had_matrix
    ...
    def forward(self, *args: Any, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._had_matrix is not None:
            ...
            k = matmul_hadU(k.float()).to(orig_dtype)
